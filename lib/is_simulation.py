...
class UnifBFBSimulation(Simulation):
    ...
    # 노드 실패 확률 반환(=노드 실패율/전체 실패율)
    # 실패 이벤트가 노드인지 디스크인지 결정
    ...
    # disk_index에 대한 디스크의 다음 복구 시간 설정
    # next repair time = get_disk_repair_duration(걸리는 시간) + get_earliest_repair_time(가장 큰 값)
